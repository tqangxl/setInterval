...
function rmHadSend(status, send, currTime, sliceArry, hadSend) {
  ...
  for (var a = 0; a < status.length; a++) {
    if (currTime - status[a].add_time >= 60) {
      send.push(status[a]);
  ...
  if (send.length) {
    ...
    console.log("Remove took: "+timeDiff+"ms");
  }
...
function mainFun(send) {
  ...
  for (var c = 0; c < send.length; c++) {
    (function (c) {
      //查询要发送商户的信息
      mysqldb.query(
        "SELECT contact_phone FROM seller WHERE seller_id=?", [send[c].seller_id],
        function (err, seller) {
          if (err) {
            hadError(err);
          }
...
var i = setInterval(CheckAlerSend, 50000);